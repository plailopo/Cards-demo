...
$.extend( Chiara, {
    ...
    reCenter: function(id) {
      ...
      Chiara.wizard.scrollBox(id, $(id).find('.c-wizard-box:eq('+cfg.currentBox+')').width() * cfg.currentBox, cfg.speed);
    ...
    next: function(id) {
      ...
      var dist = ( $(id).find('.c-wizard-box:eq(0)').width() * cfg.currentBox ) ;
			Chiara.wizard.scrollBox(id, dist, cfg.speed);
      $(id).data('c-wizard', cfg);
    },
    ...
    scrollBox: function(id, distance, duration) {
    	...
    	var cfg = $(id).data('c-wizard');
			if(!cfg) return;
      if(duration > 10){
    	   wzr.find('.c-wizard-container').css({'transition': 'transform ' + duration + 'ms cubic-bezier(0.645, 0.045, 0.355, 1)'});
      }

      //inverse the number we set in the css
      distance = -distance;
      console.log('margin', distance)
      wzr.find('.c-wizard-container').css("transform", 'translate(' + distance + "px, 0)");
